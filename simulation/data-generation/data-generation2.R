################################################################################
# data-generation2.R is used to generate the data for the Alt-1 design.
...
data_generation <- function(i) {
  ...
  gamma = c(4.5, -0.5, c(-0.05, -0.5, -0.5), c(0, -3.5, -2))
  boot_data_subcluster$lambda_prob = plogis(as.vector(as.matrix(boot_data_subcluster[, c("Intercept", "A", "log_subclusters_size", "avg_sub_sex",
                                                                                         ...
                                                                                         "A_IRS")]) %*% gamma))
  #mean(boot_data_subcluster$lambda_prob)
  ...
  eta = c(1.5, -0.5, c(0.2, 0, 0), c(0.2, 0, 0))
