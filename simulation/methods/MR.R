######################################################################################################
# MR.R is the helper function for estimating the multiply robust weights. The file contains three functions:
# (1) compute_ghat: function to compute ghat matrix for run-GEE-EM.R
# (2) compute_ghat_no_EM: function to compute ghat matrix for run-GEE-no-EM.R
...
compute_ghat_no_EM <- function(pred_prob_cluster, pred_prob_subject) {
  N <- nrow(pred_prob_cluster)
  K <- ncol(pred_prob_cluster)
  L <- ncol(pred_prob_subject)
  
  ghat <- matrix(0, nrow = L * K, ncol = N)
  index = 0
  for (i in 1:K) {
    for (j in 1:L) {
      index = index + 1
      pihats <- pred_prob_cluster[,i]
      lhats <- pred_prob_subject[,j]
      ghat[index, ] <- pihats* lhats - mean(pihats*lhats)
    }
  }
  return(ghat)
...
modNR <- function(ghat, r, maxit = 10000, eps = 10e-6, quietly = T) {
  N <- length(r)
  M <- sum(r)
